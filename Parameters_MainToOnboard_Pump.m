...
LH2Model.R2 = 0.2;  				          % [m] radius of (ET)
...
LH2Model.nV1 = 5;					% [] ST grid size (vapor)
LH2Model.tminV1 = 0.1;				% [s] ST grid time constant (vapor)
LH2Model.nV2 = 5;					% [] ET grid size (vapor)
...
LH2Model.DPipe = 0.8*inTom;              % [m] diameter of pipe
LH2Model.LPipe = 4;                   % [m] transmission line length
...
LH2Model.tau_tr = 1.5;                  % [s] transmission line delay constant !! MAY NEED TO INCREASE THIS VALUE IF ODE SOLVER CRASHES (TOO STIFF) !!
LH2Model.dE = 0.35*inTom;		       % [m] transfer line valve diameter (10 in.)
...
if Topfill
    ...
    LH2Model.ConvCoeffTopfill = 000;          
    LH2Model.bulkevap_ratio_top_bottom=0.05*(LH2Model.ConvCoeffTopfill/20000);        % ratio between top and bottom fill to (ET) at the beginning of the process. 0.5 = 50% of liquid goes to top. Values between 0.01 and 0.03 are best...
else
...
LH2Model.p_ET_low = 11*barToPa;        % [Pa] ET vent valve lower pressure threshold (= PRD hysteresis pressure)
LH2Model.p_ET_high = 12*barToPa;      % [Pa] ET vent valve upper pressure threshold (= PRD set pressure) 
LH2Model.p_ET_final = 11*barToPa;      % [Pa] final venting pressure for (ET) (= pressure in the truck's tank before leaving the station)
...
if HydrogenTransfer==0      % If hydrogen transfer is deactivated, duration is set
    ...
    LH2Model.tFinal = 1.1*60*60*(LH2Model.VTotal2/12.5)*(LH2Model.TopET-LH2Model.pct_hL20)*(30*kgminTokgsec/LH2Model.PumpMassTransferFast)*.8;  % [s] Simulation final time (adaptative with ET volume and fill percentage)
