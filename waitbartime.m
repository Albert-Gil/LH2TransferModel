...
%     H = WAITBARTIME(X,'message') creates and displays a waitbar of fractional 
%           length X.  The handle to the waitbar figure is returned in H.
%           X should be between 0 and 1. 
%  
%     WAITBARTIME(X) will set the length of the bar in the most recently
%           created waitbar window to the fractional length X.
%  
%     WAITBARTIME(X,H) will set the length of the bar in waitbar H
%           to the fractional length X.
%  
%     WAITBARTIME(X,H,'message') will update the message text in
%           the waitbar figure, in addition to setting the fractional
%           length to X.
%  
%     WAITBARTIME is typically used inside a FOR loop that performs a
%           lengthy computation.  
%  
%     Example:
%         h = waitbartime(0,'Please wait...');
%         for i=1:1000,
%             % computation here %
%             waitbartime(i/1000,h)
%         end
%
% NOTES:
